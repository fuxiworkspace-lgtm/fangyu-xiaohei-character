...
当前 Skill 必须完整继承原 `ian-xiaohei-scenes` 对人物表现力的要求，不能只继承“真实物件 + 留白”的表面风格：

- 小猫必须承担核心物理动作，不能站在旁边看、举牌解释或摆拍。
- 动作必须有接触、受力、拉扯、阻挡、修复、拖拽或被物件影响的关系。
- 角色可以因为动作轻微变瘦、变扁、变矮、变胖或歪斜，但变化必须服务动作，不能破坏黑色猫耳底盘。
- 保持空、呆、冷静、认真和略微笨拙的性格，不使用夸张表情、卖萌或职业装表演。
- 角色的可爱来自“认真做荒诞的事”，而不是大眼睛、笑脸、配饰或装饰性姿势。
- 判断标准是：如果删掉小猫，画面的隐喻仍然完全成立，说明动作设计失败。

原 Skill 的人物表现力规则来自 `references/xiaohei-ip.md`、`references/style-dna.md`、`references/object-patterns.md`、`references/prompt-template.md` 和 `references/qa-checklist.md`；本 Skill 只用固定的猫耳、黑色身体和彩色工作服覆盖角色外形，不覆盖这些动作、性格、比例弹性和表现力要求。
...
完成标记：已经选择默认小黑或彩色工作服，并能说明衣服颜色如何帮助 3 秒读懂；所有角色仍保持同一黑色猫耳形体、眼睛、细四肢和物理动作逻辑，只改变工作服颜色。
...
- 真实摄影物件自然融合，统一光影和透视。
- 小黑参与核心物理动作。
- 角色颜色路由：默认小黑或彩色工作服的选择理由；彩色角色只改变哑光工作服颜色，不改变黑色身体、猫耳、眼睛、四肢、比例、性格和动作逻辑。
- 2-4 个短中文手写标签。
- 少量蓝 / 粉 / 黄 / 绿 / 红点缀。
- 中等场景覆盖面积，但视觉重量轻。
...
- 小黑必须出现，并承担核心物理动作。
- 如使用彩色小黑，衣服颜色必须服务内容语义；所有角色必须保持同一黑色猫耳形体、白色小眼睛、细四肢和冷静认真气质，只改变工作服颜色。
